...
function Auth() {
  ...
  useEffect(() => {
    ...
    async function checkAuth() {
      ...
      if (localStorage.getItem(cons.secretKey.BASE_URL) === undefined && jwtBaseUrl === null) {
        if (!isCancelled) {
          setLoginToken(null);
          setJwtBaseUrl(null);
        }
      ...
      const baseUrl = jwtBaseUrl || localStorage.getItem(cons.secretKey.BASE_URL);
      try {
        await auth.loginWithToken(baseUrl, loginToken, jwtBaseUrl !== null);
    ...
    // Función de limpieza para configurar la cancelación
    return () => {
      isCancelled = true;
    };
